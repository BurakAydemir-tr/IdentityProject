    ...
    public class PermissionSeed
    {
        public static async Task Seed(RoleManager<AppRole> roleManager)
        {
            ...
            if (!hasAdminRole)
            {
                ...
                var AadminRole = await roleManager.FindByNameAsync("AdminRole");

                await AddReadPermission(AadminRole, roleManager);
                await AddUpdateAndCreatePermission(AadminRole, roleManager);
                await AddDeletePermission(AadminRole, roleManager);
            }
        }
        ...
        {
            await roleManager.AddClaimAsync(role, new Claim("Permission", PermissionsRoot.Permissions.Stock.Creat));
            await roleManager.AddClaimAsync(role, new Claim("Permission", PermissionsRoot.Permissions.Order.Creat));
            await roleManager.AddClaimAsync(role, new Claim("Permission", PermissionsRoot.Permissions.Catalog.Creat));
